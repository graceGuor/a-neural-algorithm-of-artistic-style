...
from neural_stylization.util.img_util import denormalize
from neural_stylization.util.img_util import matrix_to_image
...
class Adam(object):
    ...
    def __call__(self,
        ...
        self.loss_history = []
        self.learning_rates = np.logspace(np.log10(self.learning_rate), 3, iterations)
        for i in tqdm(range(iterations)):
            ...
            if save_path is not None and i % save_freq == 0:
                image = convert_func(X)
                name, suffix = save_path.split('.')
                image.save(name + str(i) + '.' + suffix)

        # print learning_rate vs loss figure
        print("self.learning_rates:", self.learning_rates)
        print("self.loss_history:", self.loss_history)
        plt.semilogx(self.learning_rates, self.loss_history)
        plt.xlabel("learning_rate")
        plt.ylabel("loss")
        plt.savefig("img/res/learning_rate_vs_loss.png")
